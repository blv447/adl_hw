...
class BSQPatchAutoEncoder(PatchAutoEncoder, Tokenizer):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        cnt = torch.bincount(self._code_to_index_flat(z), minlength=2**self.codebook_bits)
        monitoring = {
            "cb0": (cnt == 0).float().mean().detach(),
            "cb2": (cnt <= 2).float().mean().detach(),
            "cb10": (cnt <= 10).float().mean().detach(),
        }
        return x_hat, monitoring

    def _code_to_index_flat(self, z: torch.Tensor) -> torch.Tensor:
        return self.bsq._code_to_index(z).flatten()